...
section. Uncomment the hooks block in the engine definition's
`config-file` content to enable them:
...
To use the [rtk](https://github.com/nickelc/rtk) command rewriter,
...
pre-agent-steps:
  - name: Install rtk
    run: |
      curl -fsSL https://github.com/nickelc/rtk/releases/latest/download/rtk-linux-x86_64.tar.gz \
...
The `model-env-var: JCODE_MODEL` in the engine definition tells gh-aw to
pass the model via the `JCODE_MODEL` environment variable. jcode also
accepts `--model` as a CLI flag.
